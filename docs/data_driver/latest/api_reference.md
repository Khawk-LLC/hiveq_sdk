...
users; see [Migrating from KDB](#migrating-from-kdb).
...
> [`hiveq_data_api_reference.md`](hiveq_data_api_reference.md).
...
# extras:
pip install 'HiveQDataDriver[hiveq]'    # HiveQ Data API SDK (transport=HiveQ)
pip install 'HiveQDataDriver[kdb]'      # legacy kdb+ transport (qpython3)
```

**Pre-requisite:** pandas >= 1.0.
...
subscriber section on the same topic (see [Subscription support](#subscription-support)):
...
```python
A = collections.namedtuple('Input', ['date', 'sym', 'stime', 'etime'])
...
DateRangeParam = collections.namedtuple('Params', ['date', 'sym'])
date_range_param = DateRangeParam(DateRange('2025-10-13', '2025-10-17'), 'AAPL')
...
A = collections.namedtuple('A', ['date', 'sym'])
params = A(DateRange('2025-10-14', '2025-10-14'), ['AAPL', 'MSFT', 'NVDA'])
