...
- **api key** — `apiKey` in the data-source section → `apiKey` in **`[HiveQ]`** →
  `apiKey` in `[default]`.
...
Put HiveQ-wide settings (`apiKey`, `baseUrl`, `timezone`, `limit`, …) in a dedicated
**`[HiveQ]`** section so they live in one place; a per-source section still wins.
```ini
[HiveQ]
apiKey  = hq_live_xxxxxxxx
baseUrl = https://vm.hiveq.ai
```
**Exact names (they are case-sensitive):**
- Section header is **`[HiveQ]`** — capital `H` and `Q`.
- Property is **`apiKey`** (camelCase); endpoint is **`baseUrl`**.

> ⚠️ The API key is a secret. Only put it in a **gitignored** config — never in a
