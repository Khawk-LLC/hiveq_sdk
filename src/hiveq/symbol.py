...
The legacy ``<ROOT><rank>!`` notation always means the calendar-roll series, so
that is what this maps to. Canonical symbols are ``ROOT.roll.rank``: ``.c``
rolls on the expiry calendar, ``.v`` rolls when volume moves to the next
contract. To use the volume-roll series, write ``ES.v.0`` directly instead of
translating.
"""
import re
