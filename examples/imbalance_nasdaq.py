#!/usr/bin/env python3
"""Nasdaq auction imbalance data via ``on_imbalance`` and metadata routing."""
...
class NasdaqImbalanceStrategy:
    ...
    def on_start(self, ctx, event):
        ...
        logger.info("[START] waiting for nasd_imbalance data")
    ...
    def on_imbalance(self, ctx, event):
        ...
        if self.count <= 5 or self.count % 100 == 0:
            ctx.add_event_log(
                f"NASDAQ imbalance #{self.count}: side={data.side} "
...
if __name__ == "__main__":
    run = hf.run_backtest(
        strategy_configs=[
            StrategyConfig(name="NasdaqImbalanceStrategy", type="NasdaqImbalanceStrategy")
        ],
        symbols=["AAPL"],
